...
internal class ClipperOffset
{
    ...
    private PolygonLong? _destinationPolygon;
    private readonly List<PointDouble> _normals = [];
    private double _delta, _sinA, _sin, _cos;
    private double _miterLim, _stepsPerRad;

    private PointLong _lowest;
    private readonly PolygonNode _polygonNodes = new();
    ...
    public void AddPath(PolygonLong path, JoinType joinType, EndType endType)
    {
        ...
        if (_lowest.X < 0)
        {
            _lowest = new PointLong(_polygonNodes.Children.Count - 1, k);
        }
        else
        {
    ...
    public void AddPaths(PolygonLongPath paths, JoinType joinType, EndType endType)
    {
    ...
    internal static PointDouble GetUnitNormal(PointLong pt1, PointLong pt2)
    {
        ...
        foreach (var node in _polygonNodes.Children)
        {
            ...
            if (len == 1)
            {
                if (node.JoinType == JoinType.Round)
                {
                    ...
                    {
                        _destinationPolygon.Add(new PointLong(
            ...
            {
                _normals.Add(new PointDouble(_normals[len - 2]));
            }
            ...
            else if (node.EndType == EndType.ClosedLine)
            {
                ...
                {
                    _normals[j] = new PointDouble(-_normals[j - 1].X, -_normals[j - 1].Y);
                }

                _normals[0] = new PointDouble(-n.X, -n.Y);
                k = 0;
                for (var j = len - 1; j >= 0; j--)
                {
                    OffsetPoint(j, ref k, node.JoinType);
            ...
            else
            {
                ...
                else
                {
                    ...
                    k = len - 2;
                    _sinA = 0;
                    _normals[j] = new PointDouble(-_normals[j].X, -_normals[j].Y);
                ...
                _normals[0] = new PointDouble(-_normals[1].X, -_normals[1].Y);
    ...
    public void Execute(ref PolygonLongPath solution, double delta)
    {
        solution.Clear();
        FixOrientations();
        DoOffset(delta);
        ...
        {
            var r = PolygonClipper.GetBounds(_destinationPolygons!);
            var outer = new PolygonLong
            {
                new PointLong(r.Left - 10, r.Bottom + 10),
                new PointLong(r.Right + 10, r.Bottom + 10),
                new PointLong(r.Right + 10, r.Top - 10),
                new PointLong(r.Left - 10, r.Top - 10)
            };
