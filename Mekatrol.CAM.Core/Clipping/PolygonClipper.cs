...
internal class PolygonClipper
{
    ...
    private readonly List<OutputPolygon> _outputPolygons = [];
    ...
    public bool AddPath(PolygonLong polygon, PolygonKind polygonKind)
    {
#if use_lines
  ...
  if (!Closed)
    throw new ClipperException("AddPath: Open paths have been disabled.");
        ...
        if (simplifiedPolygon.Count < minVertexCount)
        {
            return false;
        }
        ...
        do
        {
            ...
            if (isFlat && edge!.Current.Y != edges[0].Current.Y)
            {
                isFlat = false;
            }
        }
    ...
    public bool AddPath(PolygonLongPath path, PolygonKind polygonKind)
    {
        var result = false;
    ...
    public bool Execute<T>(
        ClipOperation operation,
        PolygonLongPath? subject,
        PolygonLongPath? clip,
        T solution,
        bool simplify = false,
        ...
        PolygonFillType clipFillType = PolygonFillType.EvenOdd) where T : IClipSolution
    {
        ...
        switch (_solutionType)
        {
            case SolutonType.Path:
                if (solution is PolygonLongPath path)
                {
                    BuildResult(path);
                }
                return true;
            ...
            case SolutonType.Tree:
                if (solution is PolygonTree tree)
                {
                    BuildResult(tree);
                }
                return true;
            ...
            default:
                throw new Exception($"Unknown value '{_solutionType}'");
        }
    }
    ...
    private Edge ProcessBound(Edge edge, bool leftBoundIsForward)
    {
        ...
        if (result.OutIndex == ClippingHelper.Skip)
        {
            ...
            if (leftBoundIsForward)
            {
                ...
                while (edge != result && edge.IsHorizontal)
                {
                    edge = edge.Prev!;
                }
            }
    ...
    private bool BuildFlatLml(PolygonLong polygon, IReadOnlyList<Edge> edges)
    {
        var edge = edges[0];
    ...
    private bool BuildLml(PolygonLong polygon, IReadOnlyList<Edge> edges)
    {
    ...
    public static RectangleLong GetBounds(PolygonLongPath paths)
    {
        var i = 0;
        ...
        if (i == count)
        {
            return new RectangleLong(0, 0, 0, 0);
        }

        var result = new RectangleLong
        {
            Left = paths[i][0].X,
            Top = paths[i][0].Y
        };
    ...
    internal static void FixHoleLinkage(OutputPolygon outputPolygon)
    {
        //skip if an outermost polygon or
        //already already points to the correct FirstLeft ...
        if (outputPolygon.FirstLeft == null ||
              (outputPolygon.IsHole != outputPolygon.FirstLeft.IsHole &&
              outputPolygon.FirstLeft.Points != null))
        {
            return;
        }
    ...
    private bool ExecuteInternal()
    {
        ...
        while (PopScanbeam(out var topY) || _currentLocalMinima != null)
        {
            ProcessHorizontals();
            _ghostJoins.Clear();
        ...
        {
            if (outputPolygon.Points == null || outputPolygon.IsOpen)
            {
                continue;
            }
    ...
    private void AddJoin(OutputPoint point1, OutputPoint point2, PointLong offset)
    {
        var j = new Join
        {
            OutPoint1 = point1,
    ...
    private void AddGhostJoin(OutputPoint point, PointLong offset)
    {
        var j = new Join
        {
            OutPoint1 = point,
    ...
    private bool IsContributing(Edge edge)
    {
        ...
        switch (pft)
        {
            case PolygonFillType.EvenOdd:
                ...
                if (edge.WindDelta == 0 && edge.WindCount != 1)
                {
                    return false;
                }
            ...
            case ClipOperation.Intersection:
                {
                    return pft2 switch
                    {
                        PolygonFillType.EvenOdd or PolygonFillType.NonZero => edge.WindCount2 != 0,
                        PolygonFillType.Positive => edge.WindCount2 > 0,
                        _ => edge.WindCount2 < 0,
                    };
                }
        ...
        else
        {
            ...
            if (e.WindCount * e.WindDelta < 0)
            {
                ...
                if (Math.Abs(e.WindCount) > 1)
                {
                    ...
                    {
                        edge.WindCount = e.WindCount + edge.WindDelta;
                    }
                }
                else
                {
    ...
    private void AddLocalMaxPoly(Edge edge1, Edge edge2, PointLong point)
    {
    ...
    private OutputPoint AddLocalMinPoly(Edge edge1, Edge edge2, PointLong point)
    {
        OutputPoint result;
        Edge edge, prev;
        ...
        if (xPrev != xEdge ||
            ...
            !GeometryHelper.SlopesEqual(
                new PointLong(xPrev, point.Y), prev.Top, new PointLong(xEdge, point.Y), edge.Top, _useFullRange))
        {
            return result;
        }
    ...
    private OutputPoint AddOutputPoint(Edge edge, PointLong point)
    {
        if (edge.OutIndex < 0)
        {
    ...
    internal static void SwapPoints(ref PointLong point1, ref PointLong point2)
    {
        var tmp = new PointLong(point1);
        point1 = point2;
        point2 = tmp;
    }
    ...
    private static bool HorzSegmentsOverlap(long seg1A, long seg1B, long seg2A, long seg2B)
    {
        if (seg1A > seg1B)
        {
            GeometryHelper.Swap(ref seg1A, ref seg1B);
        }

        if (seg2A > seg2B)
        {
            GeometryHelper.Swap(ref seg2A, ref seg2B);
        }
    ...
    private static void ReverseLinks(OutputPoint point)
    {
        ...
        do
        {
            (polygon1.Prev, polygon1.Next) = (polygon1.Next, polygon1.Prev);
            polygon1 = polygon1.Prev!; // Prev is the orginal Next
        } while (polygon1 != point);
    }

    private static void SwapSides(Edge edge1, Edge edge2)
    {
        (edge2.Side, edge1.Side) = (edge1.Side, edge2.Side);
    }

    private static void SwapPolyIndexes(Edge edge1, Edge edge2)
    {
        (edge2.OutIndex, edge1.OutIndex) = (edge1.OutIndex, edge2.OutIndex);
    }
    ...
    private static long GetFillTypeWindingCount(PolygonFillType type, int count)
    {
        return type switch
        {
            PolygonFillType.Positive => count,
            PolygonFillType.Negative => -count,
            _ => (long)Math.Abs(count),
        };
    }
    ...
    private void IntersectEdges(Edge edge1, Edge edge2, PointLong point)
    {
        ...
        if (edge1.Kind == edge2.Kind)
        {
            ...
            {
                (edge2.WindCount, edge1.WindCount) = (edge1.WindCount, edge2.WindCount);
            }
            else
            {
        ...
        GetFillType(edge1, out var e1FillType1, out var e1FillType2);
        GetFillType(edge2, out var e2FillType1, out var e2FillType2);
    ...
    private void ProcessHorizontal(Edge horzEdge)
    {
        ...
        GetHorzDirection(horzEdge, out var edgeDirection, out var horzLeft, out var horzRight);
        ...
        while (true) //loop through consec. horizontal edges
        {
            ...
            while (edge != null)
            {
                ...
                if (currMax != null)
                {
                    if (edgeDirection == EdgeDirection.LeftToRight)
                    {
                        while (currMax != null && currMax.X < edge.Current.X)
                        {
                            ...
                            {
                                AddOutputPoint(horzEdge, new PointLong(currMax.X, horzEdge.Bottom.Y));
                            }
                            currMax = currMax.Next;
                        }
                    }
                ...
                if (edgeDirection == EdgeDirection.LeftToRight)
                {
                    IntersectEdges(horzEdge, edge, new PointLong(edge.Current.X, horzEdge.Current.Y));
                }
                else
                {
                    IntersectEdges(edge, horzEdge, new PointLong(edge.Current.X, horzEdge.Current.Y));
                }
    ...
    internal static bool IsMinima(Edge edge)
    {
        return edge != null &&
    ...
    internal static Edge? GetMaximaPair(Edge edge)
    {
    ...
    internal static Edge? GetMaximaPairEx(Edge edge)
    {
    ...
    private void BuildIntersectList(long topY)
    {
        ...
        while (isModified && _sortedEdges != null)
        {
            ...
            while (edge.NextInSel != null)
            {
                ...
                {
                    IntersectPoint(edge, nextInSel, out var point);

                    if (point.Y < topY)
                    {
                        point = new PointLong(TopX(edge, topY), topY);
                    }
    ...
    private static void IntersectPoint(Edge edge1, Edge edge2, out PointLong point)
    {
        point = new PointLong();
            ...
            else
            {
                ...
                if (SimplifySolution)
                {
                    ...
                    if (edge.OutIndex >= 0 &&
                        ...
                        prevInAel.WindDelta != 0)
                    {
                        var point = new PointLong(edge.Current);
    ...
    private void DoMaxima(Edge edge)
    {
        ...
        else if (edge.OutIndex >= 0 && maxPair.OutIndex >= 0)
        {
            if (edge.OutIndex >= 0)
            {
                AddLocalMaxPoly(edge, maxPair, edge.Top);
            }
    ...
    public static void ReversePaths(PolygonLongPath path)
    {
    ...
    private void BuildResult(PolygonLongPath path)
    {
        path.Clear();
        ...
        foreach (var outputPolygon in _outputPolygons)
        {
            ...
            var polygon = new PolygonLong(pointCount);
    ...
    private void BuildResult(PolygonTree tree)
    {
        ...
        foreach (var outputPolygon in _outputPolygons)
        {
            if (outputPolygon.PolygonNode == null)
            {
                continue;
            }
    ...
    private static bool JoinHorz(
        ...
        PointLong point, bool discardLeft)
    {
    ...
    private bool JoinPoints(Join j, OutputPolygon outputPolygon1, OutputPolygon outputPolygon2)
    {
        ...
        if (isHorizontal)
        {
            ...
            if (!GetOverlap(op1.Point.X, op1B.Point.X, op2.Point.X, op2B.Point.X, out var left, out var right))
            {
                return false;
            }
    ...
    private static int PointInPolygon(PointLong point, OutputPoint op)
    {
    ...
    private void JoinCommonEdges()
    {
        foreach (var j in _joins)
        {
            ...
            if (outputPolygon1.Points == null || outputPolygon2.Points == null)
            {
                continue;
            }

            if (outputPolygon1.IsOpen || outputPolygon2.IsOpen)
            {
                continue;
            }
            ...
            else if (OutPolygon1RightOfOutPolygon2(outputPolygon1, outputPolygon2))
            {
                hole = outputPolygon2;
            }
            else if (OutPolygon1RightOfOutPolygon2(outputPolygon2, outputPolygon1))
            {
                hole = outputPolygon1;
            }
            else
            {
                hole = GetLowermostRec(outputPolygon1, outputPolygon2);
            }

            if (!JoinPoints(j, outputPolygon1, outputPolygon2))
            {
                continue;
            }
