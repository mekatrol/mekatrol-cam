...
internal class PolygonLong : List<PointLong>
{
    ...
    public PolygonLong(IEnumerable<PointLong> points) : base(points)
    {
    }

    public PolygonLong(int capacity) : base(capacity) { }
    ...
    public Containment GeometricallyContains(PointLong point)
    {
    ...
    public PolygonLong Simplified(double epsilon = GeometryHelper.PolygonColinearityScaleConstant)
    {
        // Make copy of polygon.
        var simplified = new PolygonLong(this)
        {
            IsClosed = IsClosed
        };
    ...
    public PolygonLong Translated(PointLong offset)
    {
        var translated = new PolygonLong(Count);
        for (var i = 0; i < Count; i++)
        {
            translated.Add(new PointLong(this[i].X + offset.X, this[i].Y + offset.Y));
        }
        return translated;
    }

    public PolygonLong Cleaned(double distance = 1.415)
    {
        ...
        var result = new PolygonLong(pointCount);
